...
BB=${1:?usage: submit_pilot.sh <backbone-stem> [n_seqs]}
N=${2:-8}

mkdir -p "${AME}/logs" "${AME}/baseline"
cd "${CLEO}"

SEQS="${AME}/baseline/${BB}_n${N}.csv"

uv run python "${AME}/sample_baseline.py" \
    --pdb "${AME}/targets/${BB}-atomized-bb-True.pdb" \
    --n "${N}" --temperature 0.1 --out "${SEQS}"

uv run python "${AME}/fold_and_score.py" \
    --seqs "${SEQS}" \
    --rundir "${AME}/runs/pilot"
