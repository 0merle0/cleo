...
echo "The file $SCRIPT_PATH is being run as a shebang executable. It will...
    1. Add the "itopt" repo directory to your PYTHONPATH.
...
# Get package name from script path: assuming structure is pkg_name/exec/
SCRIPT_PARENT_DIR=$(dirname "$SCRIPT_DIR")
PKG_NAME=$(basename "$SCRIPT_PARENT_DIR")
...
then
    ...
    echo "The script $PYTHON_SCRIPT is not in the \"$PKG_NAME\" package. Are you sure sure you're using the right shebang?"
    exit 1
fi
